using UnityEngine;
using UnityEngine.XR.Interaction.Toolkit;
using UnityEngine.XR.Interaction.Toolkit.Interactables;
using UnityEngine.XR.Interaction.Toolkit.Interactors;

public class FuelPistol : MonoBehaviour, IFuelPistol
{
    [SerializeField] public Transform restPoint;
    //[SerializeField] private FuelPistolManager _anchor;
    [SerializeField] private Rigidbody rb;


    private bool isHeld = false;
    private Transform holder;

    public bool IsHeld => isHeld;

    private void Awake()
    {
        //_anchor.GetComponent<FuelPistolManager>();
        if (rb == null) rb = GetComponent<Rigidbody>();
        rb.isKinematic = true;

    }
    
    private void MetodSearch()
    {
        if (restPoint == null)
        {
            string number = name.Replace("FuelPistol", "");
            Transform pumpRoot = transform.parent;
            while (pumpRoot != null && !pumpRoot.name.StartsWith("FuelPump"))
            {
                pumpRoot = pumpRoot.parent;

                if (pumpRoot != null)
                    restPoint = pumpRoot.Find($"FuelPistolAnchor{number}");

                if (restPoint == null)
                    Debug.LogWarning($"{name}: FuelPistolAnchor{number} не найден!");
            }
        }

    }
    public void Interact()
    {
        if (isHeld) ReturnToRest();
        else AttachTo(holder); // holder должен быть задан из Interactor
    }

    public void AttachTo(Transform targetHand)
    {
        holder = targetHand;
        isHeld = true;
        transform.SetParent(holder);
        transform.localPosition = Vector3.zero;
        transform.localRotation = Quaternion.identity;

        rb.isKinematic = true;

        Collider col = GetComponent<Collider>();
        if (col != null) col.enabled = false;
    }

    public void ReturnToRest()
    {
        if (restPoint == null)
        {
            Debug.LogWarning($"{name}: restPoint не найден, возврат невозможен");
            return;
        }

        isHeld = false;
        transform.SetParent(restPoint, true);
        transform.localPosition = Vector3.zero;
        transform.localRotation = Quaternion.identity;

        rb.isKinematic = true;

        Collider col = GetComponent<Collider>();
        if (col != null) col.enabled = true;
    }
}
